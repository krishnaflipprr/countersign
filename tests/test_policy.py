...
class ClaimInputs(unittest.TestCase):
    ...
    def test_a_rewritten_input_is_a_weakening_with_the_command_unchanged(self) -> None:
        ...
        self.assertFalse(any("If a maintainer accepts this change" in s for s in receipt_json(run_gate(config, claims_base="main", approved=True))["plain"]), "once approved, the instruction is gone")
